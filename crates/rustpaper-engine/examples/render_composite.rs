...
use rustpaper_engine::program::{Blend, Program, ortho};
...
fn main() {
    let dir = std::env::args()
        .nth(1)
        .expect("uso: render_composite <pasta> [saida.png] [tempo]");
    let out_png = std::env::args()
        .nth(2)
        .unwrap_or_else(|| "/tmp/composite.png".into());
    ...
    for layer in &layout.layers {
        ...
        let Some((ltex, _tw, _th)) = load_texture(&device, &queue, &tex_bytes) else {
            continue;
        };
        ...
        let key = format!(
            "{}|{:?}|{}",
            layer.material.shader, layer.material.combos, layer.blend
        );
        if !programs.contains_key(&key) {
            let vsrc = resolve(&pkg, &format!("shaders/{}.vert", layer.material.shader))
                .and_then(|b| String::from_utf8(b).ok());
            let fsrc = resolve(&pkg, &format!("shaders/{}.frag", layer.material.shader))
                .and_then(|b| String::from_utf8(b).ok());
            let (Some(vsrc), Some(fsrc)) = (vsrc, fsrc) else {
                println!(
                    "  (pulando '{}': shader {} não encontrado)",
                    layer.name, layer.material.shader
                );
                continue;
            };
            match Program::build(
        ...
        for &b in &prog.tex_bindings {
            ...
            if let Some(name) = name
                ...
                && let Some((t, _, _)) = load_texture(&device, &queue, &bytes)
            {
        ...
        for &b in &prog.tex_bindings {
            ...
            let view = if n == 0 {
                ...
                extra_views.get(&b).unwrap_or(&white)
            };
        ...
        let mut enc = device.create_command_encoder(&Default::default());
        {
            let load = if first {
                wgpu::LoadOp::Clear(wgpu::Color {
                    r: layout.clear_color[0] as f64,
                    g: layout.clear_color[1] as f64,
                    b: layout.clear_color[2] as f64,
                    a: 1.0,
                })
            } else {
                wgpu::LoadOp::Load
