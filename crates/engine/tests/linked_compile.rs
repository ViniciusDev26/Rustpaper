...
fn linked_ubo_e_unificado() {
    ...
    let (v_spv, f_spv) = match engine::shader_compile::compile_linked(&vert, &frag, &[], &dir) {
        Ok(x) => x,
    ...
    let vr = engine::shader_compile::reflect(&v_spv).unwrap();
    let fr = engine::shader_compile::reflect(&f_spv).unwrap();
